...
# Set global variables for model training
...
def train_model(X_train: np.ndarray, y_train: np.ndarray, 
                ...
                progress_tracker: Optional[ProgressTracker] = None) -> Tuple[xgb.XGBClassifier, Dict[str, float]]:
    """
    ...
    Returns:
        Trained model and feature importance dictionary
    """
    if progress_tracker:
    ...
    if GPU_AVAILABLE:
        ...
        # Use CPU settings
        params['tree_method'] = 'hist'  # Still use histogram method for CPU (it's fast)
        params['device'] = 'cpu'
    ...
    logger.info(f"Starting training with {params['n_estimators']} trees...")
    
    # Create progress bar for estimated iterations
    with tqdm(total=params['n_estimators'], desc="Training XGBoost model", unit="tree") as pbar:
        ...
        # Update progress bar to actual iterations used
        best_iteration = getattr(model, 'best_iteration', params['n_estimators'])
        if best_iteration < params['n_estimators']:
            pbar.update(best_iteration)
            logger.info(f"Early stopping at iteration {best_iteration}")
        else:
            pbar.update(params['n_estimators'])
    
    # Log best score
    if hasattr(model, 'best_score'):
        logger.info(f"Best validation score: {model.best_score:.6f}")
