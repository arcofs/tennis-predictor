...
TODAY = date.today()
END_DATE = (TODAY + timedelta(days=1)).strftime('%Y-%m-%d')  # One day into the future
START_DATE = (TODAY - timedelta(days=13)).strftime('%Y-%m-%d')  # 14 days back (including today)
...
def parse_arguments():
    ...
    # Date range arguments - use dynamic dates as defaults
    parser.add_argument("--start-date", default=START_DATE, 
                       help=f"Start date for fetching tournaments (YYYY-MM-DD format). Default is {START_DATE} (14 days ago).")
    parser.add_argument("--end-date", default=END_DATE, 
                       help=f"End date for fetching tournaments (YYYY-MM-DD format). Default is {END_DATE} (tomorrow).")
